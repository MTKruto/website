---
title: addStoriesToAlbum
parent: /methods
metas:
  description: Add multiple stories to an album. User-only.
---

Add multiple stories to an album.<span class="select-none"> <span class="inline-flex w-fit items-center" style="font-size:12px;white-space:nowrap;word-break:keep-all;"><span class="w-fit bg-dbt select-none text-fgt" style="padding:2px 8px;border-radius:12px;">USER-ONLY</span></span> </span>
...
The identifiers of the stories to add.
...
```ts
await client.addStoriesToAlbum(chatId, albumId, storyIds);
```
